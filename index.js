...
async function run() {
    try {
        ...
        const database = client.db("myloss");
        const destinationCollection = database.collection("lossdata");
        const placeBookingCollection = database.collection("addOrder");
...
// important code
app.get("/", (req, res) => {
    res.send("hello tour we are here");
});
app.listen(port, () => {
